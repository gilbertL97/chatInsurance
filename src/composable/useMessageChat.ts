import { useLangchainServices } from '@/services/langchainServices'
import { useOllamaService } from '@/services/ollamaService'
import type { Message } from '@/types/types'
import { ref } from 'vue'

export function useMessageChat() {
  const { tryChat } = useLangchainServices() //useOllamaService()
  ...
  const messages = ref<Message[]>([
    {
      role: 'agent',
      content: 'Hola soy ESIA en que puedo ayudarte hoy ?'
    }
  ])
  ...
  const appendMessage2 = (respose: string) => {
    //const content = respose.content
    if (respose) {
      //console.log(content);
      messages.value.push({
